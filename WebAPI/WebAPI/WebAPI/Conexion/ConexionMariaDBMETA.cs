...
namespace WebApplication3.Conexiones
{
    ...
    public class ConexionMariaDBMeta
    {
        ...
        public string InsertarTabla(string database_type, string nombre, string alias, string tipo_dato, string col_null)
        {
            try
            {
                Conexion.Open();
                MySqlCommand Insertar = new MySqlCommand("INSERT INTO meta.bds (database_type, nombre, alias, tipo_dato, col_null) VALUES ('" + database_type + "','" + nombre + "', '" + alias + "', '" + tipo_dato + "', '" + col_null +  "');", Conexion);
                Insertar.ExecuteNonQuery();
                Conexion.Clone();
